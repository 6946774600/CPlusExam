...
<form class="layui-form" id="addForm" style="padding: 10px; margin-top: 15px;">
	  ...
	  <div class="layui-form-item">
	    ...
	    <div class="layui-input-block" >
	      <input style="width: 300px;"  type="text" name="loginName" id="loginName" required  lay-verify="required" placeholder="示例：20190001" autocomplete="off" class="layui-input" />
	    </div>
	  </div>
...
layui.use(['form','layer'], function(){
	  ...
	  form.on('submit(subForm)', function(data){
		  if(uuid==-1){
			  ...
			  $.ajax({
			    url : "${path }/adminTeacher/addTeacher",
			    method : "post",
	...
	$("#loginName").on('change',function(){   //教职工和学生是一张表   所以用同一个请求 进行登录名的唯一性判断
		$.post("${path}/adminStudent/studentIdOnaly",{loginName:$("#loginName").val()},function(res){
			if(res.success)
				uuid = 0;
			else{
				uuid  = -1;
				layer.msg("该教职工号已存在，请重新输入",{time: 2000});
			}
		},"json");
	})
