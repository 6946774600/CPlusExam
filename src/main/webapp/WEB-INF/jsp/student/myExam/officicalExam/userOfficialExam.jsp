...
<head>
<meta charset="UTF-8">
<title>用户正式考试页面</title>
</head>
<body class="layui-container">

<blockquote style="margin-top: 40px;" class="layui-elem-quote layui-anim layui-anim-up">正式考试</blockquote>
...
layui.use(['form','layer','util','element','table'], function(){
		    ...
		    click: function(type){
	    	  if(type === 'bar1'){
	    		  layer.confirm('确定提交考卷吗？', {icon: 3, title:'系统提示'}, function(index){
	  		    	layer.close(index);
	    			  $.post("${path }/studentOfficialExam/commitOfficialExam",{examId : "${examId }"},function(res){
	  ...
	  //考试结束时间  开始时间+考试时间
	  var endTime = formatDate(new Date("${endTime }"))
    ...
    if(date[0]+date[1]+date[2]+date[3]==0){
	  ...
	  //获取所有题型的考题id
	  $.post("${path }/studentOfficialExam/getOfficialExamItemIds",{examId : "${examId }"},function(res){
	...
	//填充单选题的考题状态表格
		table.render({
			elem: '#radio_table',
			url: '${path }/studentOfficialExam/getOfficialExamRadioType/'+examId,  //获取当前考卷单选题的状态
			page: {
				groups : 1,
	  ...
	  //多选题考题状态填充
		table.render({
			elem: '#check_table',
			url: '${path }/studentOfficialExam/getTestExamCheckType/'+examId,  //获取当前考卷单选题的状态
			page: {
				groups : 1,
				layout : ['prev', 'page', 'next','limit'],
			} ,//开启分页
	  ...
	  //填空题状态表格填充
		table.render({
			elem: '#gap_table',
			url: '${path }/studentOfficialExam/getTestExamGapType/'+examId,  //获取当前考卷单选题的状态
			page: {
				groups : 1,
				layout : ['prev', 'page', 'next','limit'],
			} ,//开启分页
	  ...
	  //判断题考题状态填充
		table.render({
			elem: '#judge_table',
			url: '${path }/studentOfficialExam/getTestExamJudgeType/'+examId,  //获取当前考卷单选题的状态
			page: {
				groups : 1,
				layout : ['prev', 'page', 'next','limit'],
			} ,//开启分页
...
function full_radio(item_index){
	layui.use(['form','layer','element'],function(){
		...
		//进行题目的填充
		$.post("${path }/studentOfficialExam/getRadioItemExam",{ item_id : itemId , examId : examId},function(res){
...
function subRadioItem(){
	...
	if($("#radio_form").find("input:checked").length == 0){
		...
		$.ajax({
			url : "${path }/studentOfficialExam/updateRadioUserOption/"+examId,
		    method : "post",
...
function full_check(item_index){
	layui.use(['form','layer','element'],function(){
		...
		//进行考题填充
		$.post("${path }/studentOfficialExam/getCheckItemExam",{ item_id : itemId , examId : examId},function(res){
...
function subCheckItem(){
	...
	if($("#check_form").find("input:checked").length < 2){
		...
		//发送ajax请求  进行该考题的用户答案录入
		$.ajax({
			url : "${path }/studentOfficialExam/updateCheckUserOption/"+examId,
		    method : "post",
...
function full_gap(item_index){
	layui.use(['form','layer','element'],function(){
		...
		//进行考题填充
		$.post("${path }/studentOfficialExam/getGapItemExam",{ item_id : itemId , examId : examId},function(res){
...
function subGapItem(){
	...
	if(temp == null || temp.length == 0){
		...
		//发送ajax请求  进行该考题的用户答案录入
		$.ajax({
			url : "${path }/studentOfficialExam/updateGapUserOption/"+examId,
		    method : "post",
...
function full_judge(item_index){
	layui.use(['form','layer','element'],function(){
		...
		//进行考题填充
		$.post("${path }/studentOfficialExam/getJudgeItemExam",{ item_id : itemId , examId : examId},function(res){
...
function subJudgeItem(){
	if($("#judge_form").find("input:checked").length == 0){
		...
		//发送ajax请求  进行该考题的用户答案录入
		$.ajax({
			url : "${path }/studentOfficialExam/updateJudgeUserOption/"+examId,
		    method : "post",
