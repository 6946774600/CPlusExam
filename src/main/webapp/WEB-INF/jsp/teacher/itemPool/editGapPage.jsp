...
layui.use(['form','upload','layer'],function(){
	...
	form.on('submit(subForm)', function(data){
  	  $.ajax({
		    url : "${path }/teacherItemList/updateGapItem/"+"${id }",
		    method : "post",
...
function form_full(form){
	$.post("${path }/teacherItemList/getGapItemById",{id : "${id }"},function(res){
		var gap = res.extend.gap;
		$("#create_time").text(gap.create_time);
		if(gap.item_imageId==-1)
			$("#img_div").remove();
		else{
			$("#img_div").show();
			$("#img_id").attr("src","/CPlusExamImage/"+gap.upLoadFile.fileUUName+gap.upLoadFile.filTemp);	
		}
		form.val("edit_form", {
			  "item_name": gap.item_name
			  ,"item_imageId": gap.item_imageId
			  ,"item_option": gap.item_option
		}); 
	},"json")
};
