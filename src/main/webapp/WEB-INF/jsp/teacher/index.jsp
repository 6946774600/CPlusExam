...
<!-- <div class="layui-fluid"> -->
	<iframe id="index_iframe" src="${path }/teacherMenu/teacherHome" scrolling="yes" frameborder="0" style="width:100%;"></iframe>
<!-- </div> -->
...
window.setInterval("reinitIframe()", 1000);  */
	...
	function renderMenu(element){
		//获取菜单树
		$.ajax({
			url : "${path }/teacherMenu/getMenuTree",
			method : 'post',
			dataType : "json",
...
layui.use(['form'], function(){
	var form = layui.form;
	//form表单数据填充
	var url = "${path }/teacherHome/getTeacherById";
	...
	form.on('submit(subForm)', function(data){
		  $.ajax({
			    url : "${path }/teacherHome/updateTeacher",
			    method : "post",
	...
	form.on('submit(subForm2)', function(data){
		layui.use('layer',function(){
			...
			if($("#new_password").val()==$("#new1_password").val()){
				 $.post("${path }/teacherHome/updatePassword",{password : $("#new_password").val() , loginName : "${loginName }"},function(res){
					  layer.alert(res.msg,{closeBtn : 0 , title : '系统提示'},function(index){
							layer.close(index);
...
function passwordChange(){
	...
	$.post("${path }/teacherHome/confimPasd",{password : $("#old_password").val() , loginName : "${loginName }"},function(res){
			if(res.success)
				userPassword = true;
			else{
				layui.use(['layer'], function(){
